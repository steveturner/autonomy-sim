...
`--ditto behavioral` is the CI-safe default and models CRDT behavior at the document/revision level: peer discovery, replica watermarks, bounded per-link propagation, pending-document counts, and convergence. `--ditto real` is feature-gated; it creates one real Ditto small peer per defender, applies defender-only `LinkState` paths to explicit TCP transports, writes the same coordination documents, and reads peer/document convergence back through the stable wire boundary.
